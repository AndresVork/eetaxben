...
server <- function(input, output) {
  ...
  output$table1kuu <- renderTable( {
   lpm <- as.integer(input$lpm)
    ...
    colnames(df) <- c("", "2017", "2018", "Muutus")
    df
  })
...
output$joonis3kuu <- renderPlot({
 ...
 output$table1aasta <- renderTable( {
